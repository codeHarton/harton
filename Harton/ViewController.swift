...
class ViewController: BaseViewController {
    ...
    private func setupTableView(){
        ...
        Observable.just([Item(name: "沙盒", type: SandBoxViewController.self),Item(name: "ISA指针", type: IsaViewController.self),Item(name: "数据库", type: DataBaseViewController.self),Item(name: "AsyncDisplayKit", type: TextureViewController.self),Item(name: "RxSwift", type: RxViewController.self),Item(name: "Big Lotto", type: BigLottoController.self)]).bind(to: tableView.rx.items(cellIdentifier: "Cell", cellType: UITableViewCell.self)){ row, ele, cell in
